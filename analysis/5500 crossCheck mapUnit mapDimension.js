...
function crossCheckUnitDim() {
  ...
  if( dups.length > 0 ) {
    ...
    // store in file
    OntoStore.storeResult( localCfg.moduleKey, localCfg.moduleName + '_error', dups );
  ...
  for( var unitSynset of unitSynsets ) {
    ...
    // collect all referenced dimension syn sets
...
if(require.main === module) {
  crossCheckUnitDim().done(); 
} else { 
 module.exports = crossCheckUnitDim; 
}
